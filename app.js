...
// ==========================================
// GLOBAL GAME STATE - برای همه کاربران یکیه
// ==========================================
// اینجا ضریب اصلی بازی که برای همه یکسان هست
let globalMultiplier = 1.00;
let globalIsPlaying = false;
let globalIsCrashed = false;
let globalCrashPoint = 0;
let globalPlayers = [];
...
// ==========================================
// UPDATE PLAYERS - نمایش بازیکنان واقعی
// ==========================================
function updatePlayers() {
    playersList.innerHTML = '';
    
    // جمع‌آوری بازیکنان از همه کاربران (شبیه‌سازی)
    const allPlayers = [];
    ...
    // بازیکنان دیگر (شبیه‌سازی شده)
    // در حالت واقعی، این اطلاعات از سرور میاد
    const fakePlayers = [
        { name: 'Player1', bet: 50, multiplier: 2.3, cashedOut: false },
        { name: 'Player2', bet: 25, multiplier: 1.8, cashedOut: false },
        { name: 'Player3', bet: 100, multiplier: 1.2, cashedOut: false }
    ];
    
    // فقط اگه بازی در حال اجراست و بازیکن دیگری هست، نمایش بده
    ...
    allPlayers.forEach((p, i) => {
        ...
        li.innerHTML = `<span>${i+1}. ${p.name}</span><span>💰 $${p.bet}</span><span>🎯 ${p.multiplier.toFixed(2)}x ${status}</span>`;
        playersList.appendChild(li);
    });
}

// ==========================================
// GENERATE CRASH POINT - مشترک برای همه
// ==========================================
