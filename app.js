// ==========================================
// AVIATOR CRASH - VERSION 3.0 (WITH 10s DELAY)
// ==========================================
...
let globalGameState = {
    ...
    isCrashed: false,
    crashPoint: 0,
    phase: 'countdown', // 'countdown' | 'playing' | 'crashed' | 'waiting'
    countdown: 10,
    roundStartTime: 0,
    chartHistory: []
};
...
function drawChart(value) {
    ...
    const step = Math.max(1, Math.floor(chartHistory.length / 12));
    for (let i = step; i < chartHistory.length; i += step) {
        const x = padding.left + (i / 100) * chartW;
        const y = padding.top + chartH - ((chartHistory[i] - minVal) / range) * chartH;
        const clampedY = Math.max(padding.top, Math.min(padding.top + chartH, y));
        ...
        const label = chartHistory[i].toFixed(2) + 'x';
    ...
    if (points.length > 0) {
        ...
        const lastLabel = chartHistory[chartHistory.length - 1].toFixed(2) + 'x';
...
// ==========================================
// GLOBAL GAME LOOP (با وقفه ۱۰ ثانیه)
// ==========================================
function globalGameLoop() {
    // ===== PHASE 1: COUNTDOWN (10 seconds for betting) =====
    if (globalGameState.phase === 'countdown') {
    ...
    if (globalGameState.phase === 'playing') {
        ...
        if (globalGameState.multiplier >= globalGameState.crashPoint) {
            ...
            // ===== WAIT 1.5 SECONDS THEN START COUNTDOWN =====
            setTimeout(() => {
                globalGameState.phase = 'countdown';
                globalGameState.countdown = 10;  // 10 seconds for betting
                globalGameState.isCrashed = false;
                chartHistory = [];
                ...
                // Reset result message after crash
                setTimeout(() => {
            ...
            }, 1500); // 1.5 second delay before countdown
        }
    }
}
