...
// TON ADDRESS & PRICE
// ==========================================
const TON_WALLET_ADDRESS = 'UQBXln8BHdlAcsyQtaLWKewRwL1iffqvwX7TaYAT0xM0bXGH';
let tonPrice = 0;

async function fetchTonPrice() {
    const priceEl = document.getElementById('tonPriceDisplay');
    if (priceEl) {
        priceEl.textContent = '🔄 در حال دریافت...';
        priceEl.className = 'price-value loading';
    }
    ...
    try {
        ...
        if (data && data.toncoin && data.toncoin.usd) {
            ...
            if (priceEl) {
                priceEl.textContent = '$ ' + tonPrice.toFixed(4) + ' USD';
        ...
        tonPrice = 1.25;
        if (priceEl) {
            priceEl.textContent = '$ ' + tonPrice.toFixed(4) + ' USD (تقریبی)';
            priceEl.className = 'price-value';
            priceEl.style.color = '#ff8c00';
        }
    }
}
...
function copyAddress() {
    const address = document.getElementById('tonWalletAddress');
    if (address) {
...
function checkDeposit() {
    ...
    updateBalanceUI();
    alert('✅ $' + depositAmount + ' به موجودی شما اضافه شد!');
    closeDepositModal();
}
...
function depositTon(amount) {
    ...
    balance += usdAmount;
    updateBalanceUI();
    alert('✅ ' + amount + ' TON ($' + usdAmount.toFixed(2) + ') شارژ شد!');
}
