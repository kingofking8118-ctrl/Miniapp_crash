...
function drawSpeedometer(value) {
    ...
    const progress = Math.min(value / maxDisplay, 1);
    ...
    ticks.forEach(tick => {
        ...
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ctx.strokeStyle = tick <= value ? '#fff' : 'rgba(255,255,255,0.2)';
        ctx.lineWidth = tick <= value ? 3 : 1.5;
        ctx.stroke();

        const labelR = radius - 28;
        const lx = cx + Math.cos(angle) * labelR;
        const ly = cy + Math.sin(angle) * labelR;
        ctx.fillStyle = tick <= value ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.2)';
        ctx.font = '9px Arial';
