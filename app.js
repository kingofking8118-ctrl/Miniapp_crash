...
// ==========================================
// GENERATE CRASH POINT USING CRYPTO
// ==========================================
function generateCrashPoint() {
    // Use crypto for random number generation
    const array = new Uint32Array(1);
    crypto.getRandomValues(array);
    const r = (array[0] / 4294967295) * 100; // 0 to 100
    
    if (r < 20) return 1.00;
    if (r < 80) return 1.01 + ((r - 20) / 60) * 0.98;
    if (r < 90) return 2.00 + ((r - 80) / 10) * 0.99;
    if (r < 97) return 3.00 + ((r - 90) / 7) * 2.99;
    return 6.00 + ((r - 97) / 3) * 8.99;
}
...
function drawChart() {
    ...
    const maxMult = Math.max(2, ...chartHistory);
    ...
    for (let i = 0; i < chartHistory.length; i++) {
        ...
        const y = h - 30 - (chartHistory[i] / maxMult) * (h - 50);
    ...
    if (isCrashed) {
        ...
        ctx.setLineDash([5, 5]);
        ctx.beginPath();
        const lastX = 50 + ((chartHistory.length - 1) / 100) * (w - 70);
    ...
    if (chartHistory.length > 0) {
        ...
        const lastX = 50 + ((chartHistory.length - 1) / 100) * (w - 70);
        const lastY = h - 30 - (last / maxMult) * (h - 50);
...
function startGame() {
    ...
    gameLoop = setInterval(gameLoopFunction, 50);
}
...
// QUICK BET BUTTONS
// ==========================================
document.querySelectorAll('.quick-bet').forEach(function(btn) {
